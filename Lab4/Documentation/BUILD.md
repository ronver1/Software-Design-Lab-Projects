...
```
Testing Frame Stack...
run\_tests: tests.c:25: test\_stack: Assertion `s.size == 2' failed.
Aborted (core dumped)
```
...
### Error: "undefined reference to `function\_name`"

```
/usr/bin/ld: game.o: undefined reference to `fs\_push'
```

**Solution:** You haven't implemented `fs\_push()` yet, or there's a typo.
...
break fs\_push          # Set breakpoint at function
run                    # Start program
...
```bash
$ gdb ./run\_tests
(gdb) break test\_stack
Breakpoint 1 at 0x1234: file tests.c, line 15.
(gdb) run
Starting program: ./run\_tests

Breakpoint 1, test\_stack () at tests.c:15
15          fs\_init(\&s);
(gdb) next
16          assert(fs\_empty(\&s));
(gdb) print s.size
$1 = 0
...
```
512 bytes in 8 blocks are definitely lost
   at malloc (vg\_replace\_malloc.c:309)
   by create\_question\_node (ds.c:15)
```
...
```c
void fs\_push(FrameStack \*s, Node \*node, int answeredYes) {
    printf("DEBUG: fs\_push called, size=%d, capacity=%d\\n", 
           s->size, s->capacity);
...
```c
void debug\_print\_node(Node \*n) {
    if (!n) {
        printf("Node: NULL\\n");
        return;
    }
...
```bash
gcc -g test\_my\_function.c ds.c -o test\_my\_function
./test\_my\_function
```
...
```bash
valgrind --tool=callgrind ./run\_tests
